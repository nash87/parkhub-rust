...
/**
 * MarbleSidebar — the original v5 sidebar, extracted from Sidebar.tsx so the
 * customization framework can swap variants while leaving the default layout
 * untouched. Visually unchanged from before; the only edit is the
 * `data-variant` hook used by tests + e2e to assert which variant rendered.
 *
 * Behaviour identical to the previous V5Sidebar — `void` mode flips to a
 * dedicated layout; the marble layout serves both `marble_light` and
 * `marble_dark` modes by relying on token cascades from tokens.css.
 */
...
function MarbleLayout({ active, onNavigate, userLabel, userEmail }: SidebarProps) {
...
function VoidLayout({ active, onNavigate, userLabel, userEmail }: SidebarProps) {
...
export function MarbleSidebar(props: SidebarProps) {
