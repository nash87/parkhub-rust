...
--background runs the gate in a detached subshell, logs to .nido/reports/
...
their own commit status contexts (nido/local-ci/full + fop/local-ci/full
for compat) when complete.

--post-status flag publishes commit status contexts for the selected profile.
Posts nido/local-ci/<profile> (canonical) and fop/local-ci/<profile> (compat).
The GitHub PR attestation gate accepts either:
...
Profiles:
  pr    Fast PR gate: format, Rust headless checks, frontend tests/build,
        ...
        touched. Set NIDO_LOCAL_CI_NO_DIFF_AWARE=1 to force every step.
...
Environment (NIDO_LOCAL_CI_* canonical; FOP_LOCAL_CI_* accepted as fallback):
  NIDO_LOCAL_CI_STATUS_REPO    override owner/repo for status post
  NIDO_LOCAL_CI_NO_DIFF_AWARE=1 disable diff-aware skipping (pr profile)
  NIDO_LOCAL_CI_REUSE_PREPUSH=1 skip Rust steps already validated by lefthook
                               pre-push hook (.fop/pre-push-validated-<sha>.json)
  NIDO_LOCAL_CI_NO_AUTO_HEAL=1 disable astro sync auto-run on missing types
  FOP_CAPACITY_WAIT_MAX_SECS   fop build queue capacity-wait timeout (default
                               1800s when MemAvailable < 6GB, else fop default)
  NIDO_LOCAL_CI_RUN_LINTERS=1  run actionlint + yamllint + zizmor + helm-validate
                               even if no workflow / helm chart files touched
  NIDO_LOCAL_CI_AUDIT_STRICT=1 fail the gate on any cargo-audit finding (default:
                               advisory; CI enforces strict)
  NIDO_LOCAL_CI_QUEUE_BIN      queue wrapper binary. Defaults to `nido build`
                               when available, otherwise `fop`.
  FOP_LOCAL_CI_QUEUE_BIN       legacy alias for NIDO_LOCAL_CI_QUEUE_BIN
  NIDO_LOCAL_CI_DIRECT=1       bypass fop build queue entirely (kernel + earlyoom
...
# ─── env-var compat: NIDO_LOCAL_CI_* canonical; FOP_LOCAL_CI_* legacy fallback ──
# Each NIDO_* var is set from its FOP_* counterpart only when the NIDO_* name is
# not already set. Sub-processes still see FOP_* names via the original env.
: "${NIDO_LOCAL_CI_STATUS_REPO:=${FOP_LOCAL_CI_STATUS_REPO:-}}"
: "${NIDO_LOCAL_CI_NO_DIFF_AWARE:=${FOP_LOCAL_CI_NO_DIFF_AWARE:-}}"
: "${NIDO_LOCAL_CI_REUSE_PREPUSH:=${FOP_LOCAL_CI_REUSE_PREPUSH:-}}"
: "${NIDO_LOCAL_CI_NO_AUTO_HEAL:=${FOP_LOCAL_CI_NO_AUTO_HEAL:-}}"
: "${NIDO_LOCAL_CI_RUN_LINTERS:=${FOP_LOCAL_CI_RUN_LINTERS:-}}"
: "${NIDO_LOCAL_CI_AUDIT_STRICT:=${FOP_LOCAL_CI_AUDIT_STRICT:-}}"
: "${NIDO_LOCAL_CI_QUEUE_BIN:=${FOP_LOCAL_CI_QUEUE_BIN:-}}"
: "${NIDO_LOCAL_CI_DIRECT:=${FOP_LOCAL_CI_DIRECT:-}}"
: "${NIDO_LOCAL_CI_SERVER_PORT:=${FOP_LOCAL_CI_SERVER_PORT:-}}"
...
#
# Order of preference:
#   1. NIDO_LOCAL_CI_SERVER_PORT  — explicit operator override (FOP_LOCAL_CI_SERVER_PORT also accepted via compat layer)
...
allocate_parkhub_server_port() {
  if [[ -n "${NIDO_LOCAL_CI_SERVER_PORT:-}" ]]; then
    printf '%s' "${NIDO_LOCAL_CI_SERVER_PORT}"
    return 0
  fi
...
if (( background )); then
  ...
  bg_log_dir="$repo_root_for_bg/.nido/reports"
  mkdir -p "$bg_log_dir"
...
# Canonical context (nido-first) + compat context posted in parallel.
context_nido="nido/local-ci/${profile}"
context_fop="fop/local-ci/${profile}"
# Canonical report path (.nido/); compat copy also written to (.fop/).
report_dir_nido="$repo_root/.nido/reports"
report_dir_fop="$repo_root/.fop/reports"
report_path="$report_dir_nido/local-ci-${profile}-${sha}.json"
report_path_fop="$report_dir_fop/local-ci-${profile}-${sha}.json"
...
# ─── pre-push hook result re-use (opt-in via NIDO_LOCAL_CI_REUSE_PREPUSH=1) ───
prepush_marker="$repo_root/.fop/pre-push-validated-${sha}.json"
prepush_validated=0
if [[ "${NIDO_LOCAL_CI_REUSE_PREPUSH:-}" == "1" ]] && [[ -f "$prepush_marker" ]]; then
...
ensure_astro_types() {
  if [[ "${NIDO_LOCAL_CI_NO_AUTO_HEAL:-}" == "1" ]]; then return 0; fi
...
post_commit_status() {
  ...
  # Post to a single context (internal helper).
  # Tolerates HTTP 422 "No commit found for SHA" — pre-push timing race.
  _post_one_context() {
    local ctx="$1"
    local out
    if ! out="$(gh api \
      ...
      if echo "$out" | grep -qE "No commit found for SHA|HTTP 422"; then
        echo "Skipping status post for ${ctx} — commit ${sha:0:8} not yet on GitHub." >&2
        return 0
      fi
      echo "$out" >&2
      return 1
  ...
  # Post nido/local-ci/<profile> (canonical) first.
  _post_one_context "$context_nido"
  # Post fop/local-ci/<profile> (compat) so the existing branch-protection
  # required check keeps passing until it is flipped to nido/local-ci/pr.
  _post_one_context "$context_fop"
}
  ...
  cat > "$report_path" <<EOF
{
  ...
  "context": "$context_nido",
  "diff_aware": {
    "enabled": $([[ "${NIDO_LOCAL_CI_NO_DIFF_AWARE:-}" == "1" ]] && echo false || echo true),
...
EOF
  # Write compat copy to .fop/reports/ so scripts/check-local-ci-report.sh
  # and any tooling still reading the legacy path keeps working.
  mkdir -p "$report_dir_fop"
  cp "$report_path" "$report_path_fop"
}
...
# NIDO_LOCAL_CI_DIRECT=1 bypasses the fop queue entirely (kernel handles memory).
# FOP_LOCAL_CI_DIRECT=1 is the legacy alias (accepted via compat layer above).
...
run_step() {
  ...
  if [[ "${NIDO_LOCAL_CI_DIRECT:-}" == "1" ]] || ! command -v "$queue_bin" >/dev/null 2>&1; then
...
mark_failure() {
  ...
  post_commit_status "failure" "nido local ${profile} failed"
}
...
if (( diff_touch_workflows )) || [[ "${NIDO_LOCAL_CI_RUN_LINTERS:-}" == "1" ]]; then
...
if (( diff_touch_design_smoke )); then
  run_step_heavy "frontend route + v5 design smoke" "NIDO_LOCAL_CI_DIRECT=1 FOP_LOCAL_CI_DIRECT=1 ./scripts/v5-design-smoke-local.sh"
else
...
if [[ -d helm/parkhub ]] && (( helm_should_run )); then
  if command -v helm >/dev/null 2>&1; then
    ...
    post_commit_status "failure" "nido local ${profile} failed: helm not installed"
    exit 1
  else
...
if [[ "$profile" == "cd" || "$profile" == "full" ]]; then
  if command -v cargo-audit >/dev/null 2>&1; then
    ...
    # NIDO_LOCAL_CI_AUDIT_STRICT=1 is set (CI enforces strictly).
    if [[ "${NIDO_LOCAL_CI_AUDIT_STRICT:-}" == "1" ]]; then
...
if [[ "$dry_run" -eq 1 ]]; then
  ...
  printf '\nlocal CI passed (nido): %s\n' "$report_path"
  printf 'local CI passed (fop compat): %s\n' "$report_path_fop"
fi
