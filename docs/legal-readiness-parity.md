...
- Each active follow-up branch must record its current head SHA, GitHub PR URL
  or local-only status, and SHA-keyed Nido/fop local CI report before push,
  merge, or release.
- A branch is not release-ready until the exact current head has required
  GitHub checks and a matching `.fop/reports/local-ci-pr-<full-sha>.json`
  generated by the Nido-first PR gate.
- Do not reuse older PR numbers, merged baseline PR evidence, or stale local
  worktree status as proof for a newer legal-readiness follow-up.
