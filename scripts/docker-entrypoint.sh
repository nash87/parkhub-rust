...
/app/parkhub-server --headless --unattended --data-dir /data --port ${PORT:-10000} &
SERVER_PID=$!

# Wait for server to be healthy
echo "Waiting for server to start..."
for i in $(seq 1 60); do
    if wget -q --spider http://127.0.0.1:${PORT:-10000}/health 2>/dev/null; then
...
if [ "${DEMO_MODE}" = "true" ]; then
    ...
    if [ "$LOT_COUNT" -le 1 ]; then
        ...
        /app/parkhub-server --headless --data-dir /data --port ${PORT:-10000} &
        SERVER_PID=$!
        ...
        # Run the seed script
        python3 /app/seed_demo.py --base-url http://127.0.0.1:${PORT:-10000} --admin-password "${PARKHUB_ADMIN_PASSWORD:-ParkHub2026!}" 2>&1 || echo "WARNING: Demo seeding failed"
